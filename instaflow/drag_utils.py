...
@dataclass
class DragOptimStep:
    """Record of a single optimization step during drag."""
    step_idx: int
    loss: float
    handle_points: List[Tuple[float, float]]  # Current handle positions
    target_points: List[Tuple[float, float]]  # Target positions (fixed)
    latent: Optional[torch.Tensor] = None  # Detached latent for later decoding/inference


@dataclass
class DragOutput:
    """Output of drag optimization with optional visualization data."""
    latent: torch.Tensor
    optim_steps: List[DragOptimStep] = field(default_factory=list)
    converged: bool = False
    final_handle_points: List[Tuple[float, float]] = field(default_factory=list)
    total_steps: int = 0
...
def drag_rf_update(
    ...
    show_optim_process: bool = False,  # Whether to collect visualization data
    vis_interval: int = 10,            # Collect visualization every N steps
) -> DragOutput:
    """
    ...
    optim_steps: List[DragOptimStep] = []
    converged = False
    target_points_list = [(float(tp[0]), float(tp[1])) for tp in target_points]
    ...
    for step_idx in range(args.n_pix_step):
        ...
        if show_optim_process and (step_idx % vis_interval == 0):
            handle_list = [(float(hp[0]), float(hp[1])) for hp in handle_points]
            optim_steps.append(DragOptimStep(
                step_idx=step_idx,
                loss=0.0,  # Will update after loss computation
                handle_points=handle_list,
        ...
        # Update loss value in last recorded step
        if show_optim_process and optim_steps and optim_steps[-1].step_idx == step_idx:
    ...
    final_handle_list = [(float(hp[0]), float(hp[1])) for hp in handle_points]
    
    return DragOutput(
        latent=latent.detach(),
        optim_steps=optim_steps,
        converged=converged,
        final_handle_points=final_handle_list,
